...
**第三次评估结果对比** (RECALL=15, RRF_K=30):

| 指标 | 初始 | Round 1 | Round 2 | Round 3 | 总变化 |
|------|------|---------|---------|---------|--------|
| **Overall** | 0.469 | 0.485 | 0.507 | **0.520** | +0.051 |
| **NDCG** | 0.412 | 0.412 | 0.419 | **0.425** | +0.013 |
| **CR** | 0.520 | 0.520 | 0.640 | **0.640** | +0.120 |
| **Faithfulness** | 0.480 | 0.520 | 0.540 | **0.640** | +0.160 |
| **Avg Latency** | 15,582ms | 13,195ms | 17,523ms | **15,874ms** | +292ms |

**主要改善**:
- Faithfulness 大幅提升 0.48→0.64 (+33.3%)
- Q1 F: 0.400→1.000, Q2 F: 0.800→1.000, Q3 F: 0.800→1.000
- Q9 CR 保持 1.0

**Q10 NDCG 持续 regression** (0.438→0.312→0.312):
- 可能原因：英文多区域部署内容检索困难，非配置可解决
- 建议：优化 enterprise_k8s.md 英文内容的 chunk 策略
...
RRF_K=30               # 40 → 20 → 60 → 30 (平衡sharp与温柔)
```
