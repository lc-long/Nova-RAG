...
- OCR 结果通过 `merge_ocr_into_text` 合并到对应页面的文本中
- OCR 结果基于文件 MD5 哈希缓存，避免重复调用

**位置**: `backend/src/core/chunker/pdf_parser.py`, `backend/src/core/ocr/__init__.py`
...
- **距离度量**: cosine 距离
- **表结构**: `document_chunks` 表存储 chunk 内容、embedding、metadata
- **端口**: 5433 (Docker 容器)
...
| Retrieval (检索) | HybridRetriever (向量 + BM25 + RRF) | ✅ |
